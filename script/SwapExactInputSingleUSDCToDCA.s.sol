...
contract SwapExactInputSingleUSDCToDCA is Script {
    ...
    function run() external {
        ...
        // Set swap parameters
        // zeroForOne: true to swap currency0 (USDC) for currency1 (DCA); false to swap currency1 (DCA) for currency0 (USDC)
        bool zeroForOne = true;
        uint128 amountIn = 10e6; // 10 USDC tokens (assuming 6 decimals for USDC)
        uint128 minAmountOut = 0; // Set your minimum output amount

        // Approve USDC token spending using Permit2
        _approveTokenWithPermit2(USDC_TOKEN, uint160(amountIn), uint48(block.timestamp + 1000));
        ...
        params[1] = abi.encode(Currency.wrap(USDC_TOKEN), amountIn); // Settle USDC
        params[2] = abi.encode(Currency.wrap(DCA_TOKEN), minAmountOut); // Take DCA
        ...
        // Exchange rate: How many DCA (18 decimals) for 1 USDC (6 decimals), result scaled by 1e18 for logging
        if (usdcSpent > 0) {
            uint256 rate = (dcaReceived * 1e18 * 1e6) / (usdcSpent * 1e18); // (DCA_val * 10^18 * 10^6) / (USDC_val * 10^18)
            console.log("Exchange rate: 1 USDC = %s DCA (scaled by 1e18)", rate);
        } else {
            console.log("Exchange rate: Cannot calculate, USDC spent is zero.");
        }
    }
} 