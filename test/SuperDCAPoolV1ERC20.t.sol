...
contract SuperDCAPoolV1ERC20Test is Test {
  ...
  function testFork_ExecutorReceivesCorrectFeeShare() public {
    ...
    // --- Open a sizable USDCx stream from Bob to generate output ---
    uint256 highFlowRate = INFLOW_RATE_USDC * 10;
    _createFlow(bob, USDCX, address(pool), uint96(highFlowRate));

    // Let the stream run for a day so we have something to swap
    skip(1 days);
    ...
    bytes32 swapSig = keccak256("Swap(uint256,uint256,uint256,uint256,address)");
