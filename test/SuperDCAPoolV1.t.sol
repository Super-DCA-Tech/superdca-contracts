...
import {ISETHCustom} from "@superfluid-finance/ethereum-contracts/interfaces/tokens/ISETH.sol";
...
import {IConstantFlowAgreementV1} from "@superfluid-finance/ethereum-contracts/interfaces/agreements/IConstantFlowAgreementV1.sol";
import {IInstantDistributionAgreementV1} from "@superfluid-finance/ethereum-contracts/interfaces/agreements/IInstantDistributionAgreementV1.sol";
...
// OPTIMISM
import "./utils/optimism/Constants.sol";
...
contract SuperDCAPoolV1Test is Test {
    ...
    function _approveSubscription(
        ...
        IInstantDistributionAgreementV1 ida = IInstantDistributionAgreementV1(
            IDA_SUPERFLUID
        );
    ...
    function _deleteFlow(address sender) internal {
        vm.prank(sender);
        ICFAForwarder(CFA_FORWARDER).deleteFlow(
            ISuperToken(USDCX),
            sender,
            address(pool),
            new bytes(0)
    ...
    function testFork_CalculatesNextDistributionTime() public {
        ...
        uint256 nextDistTime = pool.getNextDistributionTime(
            gasPrice,
            gasLimit,
            tokenToWethRate
        );
        ...
        assertEq(
            ...
            pool.lastDistributedAt() +
                ((gasPrice * gasLimit * tokenToWethRate) /
                    (INFLOW_RATE_USDC / 1e9)) /
                1e9
        );
    }
    ...
    function testFork_GelatoDistribution() public {
        ...
        moduleData.args[1] = abi.encode(
            ...
            abi.encode(uint128(gelatoBlockTimestamp), uint128(60_000)) // Second arg (encodedArgs) in
            // TypeScript encoding
        );
    ...
    function testFork_GetIDAShares() public {
        ...
        _createFlow(alice, USDCX, address(pool), uint96(INFLOW_RATE_USDC));
        // Check IDA shares
        (bool exist, bool approved, uint128 units, uint256 pendingDist) = pool
            .getIDAShares(alice);
        assertTrue(exist);
        assertTrue(approved); // We approved in setup
        assertEq(units, uint128(INFLOW_RATE_USDC / SHARE_SCALER)); // Flow rate divided by scaler
    ...
    function testFork_CloseStream() public {
        ...
        ISuperToken(USDCX).transfer(
            bob,
            balance - (uint256(INFLOW_RATE_USDC) * 4 hours)
        );
        vm.stopPrank();
        // Should be able to close stream now
        pool.closeStream(alice, ISuperToken(USDCX));
        // Verify stream is closed
        (, int96 flowRate, , ) = IConstantFlowAgreementV1(CFA_SUPERFLUID)
            .getFlow(ISuperToken(USDCX), alice, address(pool));
        assertEq(flowRate, 0);
    }

    function testFork_RevertWhen_CloseStream() public {
        // Create flow for alice
        _createFlow(alice, USDCX, address(pool), uint96(INFLOW_RATE_USDC));
        // Should revert when trying to close stream with sufficient balance
        vm.expectRevert(NotClosable.selector);
        pool.closeStream(alice, ISuperToken(USDCX));
    }
    ...
    function testFork_ExecutionFeeShareAdjustment() public {
        ...
        uint256 maxFee = MAX_FEE_SHARE;
        uint256 minFee = MIN_FEE_SHARE;
        // Test max cap
        skip(24 hours);
    ...
    function testFork_InitializeWithEmptyRegistrationKey() public {
        ...
        vm.startPrank(AUTHORIZED_DEPLOYER, AUTHORIZED_DEPLOYER);
        SuperDCAPoolV1 newPool = new SuperDCAPoolV1(
            payable(GELATO_AUTOMATE),
            UNIVERSAL_ROUTER,
            POOL_MANAGER,
            PERMIT2
    ...
    function testFork_BeforeAgreementCreatedTokenValidation() public {
        ...
        vm.expectRevert(InvalidHost.selector);
        pool.beforeAgreementCreated(
            ISuperToken(USDCX),
            CFA_SUPERFLUID,
            bytes32(0),
    ...
    function testFork_DistributeWithContext() public {
        ...
        assertGt(
            aliceBalance,
            bobBalance,
            "Alice should have more WETHx than Bob"
        );
        vm.stopPrank();
    }
    ...
    function testFork_EmitsErrorEventOnRefundFailure() public {
        ...
        emit SuperDCAPoolV1.ErrorRefundingUninvestedAmount(
            alice,
            INFLOW_RATE_USDC * 1 hours
        );
    ...
    function testFork_EmitsRefundEventOnSuccess() public {
        ...
        vm.expectEmit(true, true, true, true);
        emit SuperDCAPoolV1.RefundedUninvestedAmount(
            alice,
            INFLOW_RATE_USDC * 1 hours
        );
    ...
    function testFork_BasicStaking() public {
        ...
        // Approve and stake
        vm.startPrank(alice);
        IERC20(pool.STAKING_TOKEN_ADDRESS()).approve(
            address(pool),
            stakeAmount
        );
    ...
    function testFork_TakeOverStake() public {
        ...
        uint256 aliceBalanceBeforeTakeover = IERC20(
        ...
        uint256 aliceBalanceAfterTakeover = IERC20(pool.STAKING_TOKEN_ADDRESS())
            .balanceOf(alice);
        assertEq(
    ...
    function testFork_ExecutorEarnsFees() public {
        ...
        assertGt(
            aliceFinalBalance,
            aliceInitialBalance,
            "Executor should receive fees in ETH"
        );
    }
    ...
    function testFork_UnstakeRestrictions() public {
        ...
        pool.stake(stakeAmount);
        uint256 aliceBalanceBeforeUnstake = IERC20(pool.STAKING_TOKEN_ADDRESS())
            .balanceOf(alice);
        vm.stopPrank();
        // // Bob shouldn't be able to         foundryup --version 1.1.0unstake (wasn't previous executor)
        // vm.startPrank(bob);
        ...
        // Verify Alice received her stake back
        uint256 aliceBalanceAfterUnstake = IERC20(pool.STAKING_TOKEN_ADDRESS())
            .balanceOf(alice);
        assertEq(
