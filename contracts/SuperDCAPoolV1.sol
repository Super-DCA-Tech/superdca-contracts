...
contract SuperDCAPoolV1 is SuperAppBase, AutomateTaskCreator {
    ...
    // Staking Variables
    address public constant STAKING_TOKEN_ADDRESS =
    ...
    function _initializeUniswap(
        ...
        for (uint256 i = 0; i < uniswapPath.length; i++) {
            if (i == uniswapPath.length - 1) {
                encodedSwapPath = abi.encodePacked(
                    encodedSwapPath,
                    uniswapPath[i]
                );
            } else {
        ...
        encodedGasPath = abi.encodePacked(
            address(weth),
            GELATO_GAS_POOL_FEE,
    ...
    function distribute(
        ...
        uint256 amountInMaximum = (underlyingBalance *
        ...
        if (!ignoreGasReimbursement) {
            ...
            IERC20(underlyingInputToken).transfer(
                currentExecutor,
                amountInMaximum
            );
        }
    ...
    function getExecutionFeeShare(
        uint256 _currentFeeShare
    ) public view returns (uint256 _adjustedFeeShare) {
        // solhint-disable-next-line not-rely-on-time
        uint256 _timeSinceLastDistribution = block.timestamp -
            lastDistributedAt;

        if (_timeSinceLastDistribution > distributionInterval) {
            uint256 _hoursPastInterval = (_timeSinceLastDistribution -
                distributionInterval) / 1 hours;
            _hoursPastInterval = _hoursPastInterval > MAX_HOURS_PAST_INTERVAL
                ? MAX_HOURS_PAST_INTERVAL
                : _hoursPastInterval;
            ...
            _adjustedFeeShare =
                _currentFeeShare *
                (GROWTH_FACTOR ** _hoursPastInterval);
    ...
    function stake(uint256 amount) external {
        ...
        ERC20(STAKING_TOKEN_ADDRESS).transferFrom(
            msg.sender,
            address(this),
            amount
        );
        ...
        if (previousExecutor != address(0)) {
            ERC20(STAKING_TOKEN_ADDRESS).transfer(
                previousExecutor,
                previousStake
            );
