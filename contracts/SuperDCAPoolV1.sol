...
contract SuperDCAPoolV1 is SuperAppBase, AutomateTaskCreator, SuperDCAPoolStaking, SuperDCASwap, Ownable {
  ...
  constructor(
    ...
    address _permit2,
    address _dcaTradeAddress
  )
    AutomateTaskCreator(_ops)
    SuperDCASwap(_router, _poolManager, _permit2)
  {
    // Use provided SuperDCATrade address or deploy new instance
    if (_dcaTradeAddress != address(0)) {
      dcaTrade = SuperDCATrade(_dcaTradeAddress);
    } else {
    ...
    returns (bytes memory newCtx)
  {
    ...
    // The amount of output tokens (in ETH terms) that represent the executor/Gelato fee share.
    // e.g. If gelatoFeeShare = 1e16 (1%) and we received 100 ETH worth of output, the executor
    // is entitled to 1 ETH. We purposefully *do not* use the complement so that we never exceed
