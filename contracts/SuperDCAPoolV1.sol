...
// Superfluid imports
import {ISuperfluid, ISuperAgreement, SuperAppDefinitions} from "@superfluid-finance/ethereum-contracts/contracts/interfaces/superfluid/ISuperfluid.sol";
    ...
    SuperDCASwap
{
    ...
    event ErrorRefundingUninvestedAmount(
        address shareholder,
        uint256 uninvestAmount
    );

    // --- Constructor ---

    // SuperDCAPoolStorage internal superDCAPoolStorage;
    ...
    constructor(
        ...
        address _permit2
    ) AutomateTaskCreator(_ops) SuperDCASwap(_router, _poolManager, _permit2) {
    ...
    function distribute(
        ...
        uint256 maxFeeEthAmount = (outputTokenAmount *
            (EXEC_FEE_SCALER - gelatoFeeShare)) / EXEC_FEE_SCALER;
    ...
    function _downgradeInputTokens(uint256 amount) internal returns (uint256) {
        if (
            underlyingInputToken != address(inputToken) &&
            underlyingInputToken != address(weth)
        ) {
    ...
    function _swap(uint256 amount) internal returns (uint256) {
        ...
        path[1] = PathKey({
            intermediateCurrency: Currency.wrap(
                outputIsETH ? ETH_ADDRESS : outputAddr
            ),
            fee: DCA_ETH_KEY.fee,
        ...
        Currency currencyIn = Currency.wrap(
            inputIsETH ? ETH_ADDRESS : inputAddr
        );
    ...
    function _handleTokenUpgrade() internal {
        if (underlyingOutputToken != address(outputToken)) {
            if (outputToken == wethx) {
                ...
                ISETH(address(outputToken)).upgradeByETH{
    ...
    function _updateShareholder(
        ...
        uint128 userShares = ShareMathLib.flowRateToShares(
            _shareholderUpdate.currentFlowRate
        );
    ...
    function _getShareAllocations(
        ...
        userShares = ShareMathLib.flowRateToShares(
            _shareholderUpdate.currentFlowRate
        );
    }
    ...
    function _calcUserUninvested(
        ...
        return
            ShareMathLib.calcUserUninvested(
                prevUpdateTimestamp,
                flowRate,
                lastDist
            );
    ...
    function getExecutionFeeShare(
        uint256 currentFeeShare
    ) public view returns (uint256) {
        return
            FeeRetargetLib.adjustFeeShare(
                currentFeeShare,
                lastDistributedAt,
    ...
    function getTradeInfo(
        address _trader,
        uint256 _tradeIndex
    ) public view returns (SuperDCATrade.Trade memory trade) {
    ...
    function getLatestTrade(
        ...
        if (dcaTrade.tradeCountsByUser(_trader) > 0)
            trade = dcaTrade.getLatestTrade(_trader);
    ...
    function closeStream(address streamer, ISuperToken token) public {
        ...
        if (int256(token.balanceOf(streamer)) > streamerFlowRate * 8 hours)
            revert NotClosable();
